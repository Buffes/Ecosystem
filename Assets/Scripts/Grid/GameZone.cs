...
namespace Ecosystem.Grid
{
    public class GameZone : MonoBehaviour 
    {
        ...
        private float waterNeighbourRate = 0.7f;
        private float greenScenaryNeigbourRate = 0.3f;

        //The rate of objects spawning
        private float waterSpawnRate = 0.005f;
        private float greenScenarySpawnRate = 0.1f;
        ...
        private void InitObjects()
        {
            ...
            bush = Resources.Load("bush1") as GameObject;
            tree = Resources.Load("tree1") as GameObject;
            rock = Resources.Load("rock1") as GameObject;
            cactus = Resources.Load("cactus1") as GameObject;
        }
            ...
            for (int row = 0; row < tiles.GetLength(0); row++)
            {
                for (int col = 0; col < tiles.GetLength(1); col++)
                {
                    if (tiles[row, col] == 34)
                    {
                        ...
                        gameObjectsInGrid[row, col] = RandomizeDesert(desertScenarySpawnRate, row, col);
                    }
                    else if (tiles[row, col] == 51)
                    {
                        if (ScenaryAsNeighbour(row, col) != Scenary.Empty)
                        {
                            gameObjectsInGrid[row, col] = RandomizeGreen(greenScenaryNeigbourRate, row, col);
                        }
                        else 
                        {
                            gameObjectsInGrid[row, col] = RandomizeGreen(greenScenarySpawnRate, row, col);
                        }
                    }
                }
            }
        ...
        private Scenary RandomizeGreen(float value, int row, int col)
        {
            ...
            if (rand <= newBush)
            {
                Instantiate (bush, new Vector3(row, 0, col), Quaternion.identity);
                walkableTiles[row, col] = false;
                return Scenary.Bush;
            }
            else if (rand <= newTree)
            {
                Instantiate (tree, new Vector3(row, 0, col), Quaternion.identity);
                walkableTiles[row, col] = false;
                return Scenary.Tree;
            }
            else if (rand <= newRock) 
            {
                Instantiate (rock, new Vector3(row, 0, col), Quaternion.identity);
                walkableTiles[row, col] = false;
                return Scenary.Rock;
            }
        ...
        private Scenary RandomizeDesert(float value, int row, int col) 
        {
            ...
            {
                Instantiate (cactus, new Vector3(row, 0, col), Quaternion.identity);
                walkableTiles[row, col] = false;
